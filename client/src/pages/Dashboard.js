...
const Dashboard = () => {
  ...
  const fetchDashboardData = useCallback(async () => {
    try {
      const [taskRes, projectRes, attRes, todayRes, weekLogRes] =
        await Promise.all([
          DashboardAPI.getTaskSummary(),
          DashboardAPI.getProjectSummary(),
          DashboardAPI.getAttendanceStatus(),
  ...
  useEffect(() => {
    ...
    if (isClockedIn && attendance?.clockIn && !attendance?.clockOut) {
      const initial = Math.floor(
        (Date.now() - new Date(attendance.clockIn)) / 1000
      );
  ...
  const handleActiveDay = useCallback(
    (day) => {
      ...
      setActiveDayData((prev) => {
        ...
        const totalMins = selected.duration + selected.break;
        setDurationPercent(
          totalMins ? (selected.duration / totalMins) * 100 : 0
        );
  ...
  useEffect(() => {
    if (weeklyLogs.length > 0 && !activeDayData) {
      const firstActive = weeklyLogs.find((d) => d.active) || weeklyLogs[0];
      if (firstActive) {
        handleActiveDay(firstActive.day);
      }
          ...
          <div className="card p-3 shadow-sm h-100 d-flex flex-column justify-content-between">
            <div className="text-end mb-2">
              ...
              {attendance?.clockOut && (
                <div className="small text-muted">
                  🔴 Clocked out at:{" "}
                  {moment(attendance.clockOut).format("hh:mm A")}
                </div>
              )}
              ...
              {attendance?.clockIn && attendance?.clockOut && (
                <div className="small text-muted mt-2">
                  ...
                  {moment
                    .utc(
                      moment(attendance.clockOut).diff(
                        moment(attendance.clockIn)
                      )
                    )
                    .format("HH:mm:ss")}
                </div>
              ...
              recentAwards.map((award, index) => (
                ...
                >
                  ...
                  <div className="d-flex align-items-center">
                    <img
                      src={
                        award?.employee?.profileImage
                          ? award.employee.profileImage.startsWith("http")
                            ? award.employee.profileImage
                            : `${
                                process.env.REACT_APP_API_URL ||
                                "http://localhost:5000"
                              }/${award.employee.profileImage}`
                          : award?.employee?.gender === "Female"
                          ? "/avatars/female.png"
                          : "/avatars/male.png"
                      }
                      alt={award.employee?.name}
          ...
          <div className="card p-3 shadow-sm h-100">
            ...
            {activeDayData && (
              <>
                ...
                <div className="d-flex justify-content-between small text-muted">
                  <span>
                    Duration: {activeDayData.durationH}h{" "}
                    {activeDayData.durationM}m
                  </span>
                  <span>
                    Break: {activeDayData.breakH}h {activeDayData.breakM}m
                  </span>
        ...
        <div className="row mb-4">
          <div className="col-md-8">
            <div className="card p-3 shadow-sm h-100">
              <h6>My Active Timer</h6>
              <div>
                Start Time: {moment(attendance.clockIn).format("hh:mm A")}
              </div>
              <div>
                Duration:{" "}
                {moment
                  .utc(moment(now).diff(moment(attendance.clockIn)))
                  .format("HH:mm")}
              </div>
            </div>
          </div>
